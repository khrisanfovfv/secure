...
class Employee
{
    ...
    function secure_search_employee()
    {
        ...
        foreach ($users as $user) {
            ...
            if (strpos(mb_strtolower($user->user_login), $value) !== false) {
                $exist = true;
            }
            ;
            if (strpos(mb_strtolower($user->last_name), $value) !== false) {
                $exist = true;
            }
            ;
            if (strpos(mb_strtolower($user->first_name), $value) !== false) {
                $exist = true;
            }
            ;
            if (strpos(mb_strtolower($user->middle_name), $value) !== false) {
                $exist = true;
            }
            ;
            if (strpos(mb_strtolower($organization_name), $value) !== false) {
                $exist = true;
            }
            ;
            if (strpos(mb_strtolower($department_name), $value) !== false) {
                $exist = true;
            }
            ;
            if (strpos(mb_strtolower($user->user_email), $value) !== false) {
                $exist = true;
            }
            ;
    ...
    function secure_search_employee_extended()
    {
        ...
        //     $results = array();
        //     $prefix = $wpdb->prefix;
        //     $login_query = $wild . $wpdb->esc_like($_POST['login']) .$wild;
        //     $last_name_query = $wild . $wpdb->esc_like($_POST['last_name']) .$wild;
        //     $first_name_query = $wild . $wpdb->esc_like($_POST['first_name']) .$wild;
        //     $middle_name_query = $wild . $wpdb->esc_like($_POST['middle_name']) .$wild;
        //     $organization_query = $_POST['organization_id'] !='' ? 'organization.id = ' . $_POST['organization_id'] : '';
        //     $department_query = $_POST['department_id'] !='' ? 'department.id = ' . $_POST['department_id'] : '';
        //     $email_query = $wild . $wpdb->esc_like($_POST['email']) .$wild;
        //     $state = $_POST['state'] != '' ? 'usermeta.user_state = ' . $_POST['state'] : '';


        // $results = $wpdb->get_results(
        //     $wpdb->prepare( 
        //         "SELECT users.user_login, usermeta.last_name, usermeta.first_name, usermeta.middle_name,
        //         organization.fullname, department.name, users.user_email, usermeta.user_state  from {$prefix}users users 
        //         JOIN {$prefix}usermeta usermeta on usermeta.user_id = user.id
        //         JOIN {$prefix}organization organization on organization.id = usermeta.organization
        //         JOIN {$prefix}department department on department.id = usermeta.department"), ARRAY_A
        // ); 
